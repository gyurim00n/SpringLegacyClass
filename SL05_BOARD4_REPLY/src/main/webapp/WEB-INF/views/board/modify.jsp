	...
	<div>
		...
		<form action="/board/modify" method="post">
			...
			<input type="hidden" name="${ _csrf.parameterName }"
				value="${ _csrf.token }">
			<input type="hidden"
				name="pageNum" value="${criteria.pageNum}">
			...
			$("tfoot button").on(
					...
					function() {
						...
						if (operation == "modify") {
							...
							const pageNumClone = $(":hidden[name='pageNum']")
									.clone();
							const pageAmountClone = $(":hidden[name='amount']")
									.clone();
							formObj.attr({
								"action" : "/board/list",
								"method" : "get"
							...
							.append(pageNumClone).append(pageAmountClone)
									.submit();
						}
